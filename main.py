...
def assure_path_exists(path):
    dir = os.path.dirname(path) #belirtilen yoldan dizin adını almak için
...
def tick():
    time_string = time.strftime('%H:%M:%S') #bilgisayardan yerel saati almak için
    clock.config(text=time_string) #zaman dizisi değiştiyse, güncellemek için
    clock.after(200,tick)



def contact():
    mess._show(title='Contact us', message="Please contact us on : 'dilaykanber@hotmail.com' ")
...
def change_pass():
    ...
    master.resizable(False,False) # pencerenin yeniden boyutlandırılmasını yasaklamak için.
...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        cam = cv2.VideoCapture(0)#Gerçek zamanlı video yakalama 
        #tanıyıcıyı ve yüz algılayıcıyı başladı
        harcascadePath = "haarcascade_frontalface_default.xml"
        detector = cv2.CascadeClassifier(harcascadePath)
        sampleNum = 0
        while (True):
            ret, img = cam.read()
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #BGR görüntüsünü gri tonlamaya dönüştürür
            faces = detector.detectMultiScale(gray, 1.3, 5) #resimdeki yüzleri bulmak için
            #Algılanan her yüz için sınırlayıcı kutu alır.
            for (x, y, w, h) in faces:
                cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
                # Artan örnek numarası
                sampleNum = sampleNum + 1
                # Yakalanan yüzü veri kümesi klasörüne kaydetme (Eğitim Görüntüsü)
            ...
            if cv2.waitKey(100) & 0xFF == ord('q'):
                break
            # örnek sayısı 100'den fazlaysa kırılma
            elif sampleNum > 100:
                break
        cam.release()
...
def TrainImages():
    check_haarcascadefile()
    assure_path_exists("TrainingImageLabel/")
    recognizer = cv2.face_LBPHFaceRecognizer.create() #tanıyıcıyı ve yüz algılayıcıyı başlatmak için
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector = cv2.CascadeClassifier(harcascadePath)
...
def getImagesAndLabels(path):
    ...
    Ids = []
    # Tüm görüntü yolları arasında dolaşıp kimlikleri ve görüntüleri yüklüyor.
    for imagePath in imagePaths:
        # resmin yüklenmesi ve gri skalaya dönüştürülmesi
        pilImage = Image.open(imagePath).convert('L')
        # PIL görüntüsünü numpy dizisine dönüştürüyor
        imageNp = np.array(pilImage, 'uint8')
        # resimden kimliği alma
        ID = int(os.path.split(imagePath)[-1].split(".")[1])
        # eğitim görüntüsü örneğinden yüzü çıkar.
...
def TrackImages():
    ...
    while True:
        ...
        faces = faceCascade.detectMultiScale(gray, 1.2, 5) #yüzü eğitim görüntüsü örneğinden çıkarmak için.
        for (x, y, w, h) in faces: #Bir yüz varsa, onu ve kimliğini listeye ekleme.
...
message3 = tk.Label(window, text=" Attendance System " ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
datef = tk.Label(frame4, text = day+"-"+mont[month]+"-"+year+"  |  ", fg="purple",bg="#262523" ,width=55 ,height=1,font=('times', 22, ' bold '))
datef.pack(fill='both',expand=1)

clock = tk.Label(frame3,fg="purple",bg="#262523" ,width=55 ,height=1,font=('times', 22, ' bold '))
